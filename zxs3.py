...
def get_ysl_id():
    try:
        ...
        if "没有可以预定的阅览室." in response.json()["content"]:
            ...
            #     return yls_id
            i = 0
            while "排队中请等待" in response.json()["content"]:
                logger.debug("正在排队中: " + str(response.json()["content"]))
                response = requests.post(url + "/seat/get_task_status?serialno=" + serial_no)
                time.sleep(1)
                i = i + 1
                if i >= 5:
...
def check_position(user_info, yls_id):
    try:
        ...
        st = time.time()
        while "没有座位." in response.json()["content"]:
            response = requests.post(url + '/seat/get_task_status?serialno=' + ser_no2)
            ed = time.time()
            # 持续200秒后退出
            if (ed - st) >= 30:
        ...
        i = 0
        while "排队中" in response.json()["content"]:
            logger.debug("getting seat no : " + str(response.json()["content"]))
            response = requests.post(url + '/seat/get_task_status?serialno=' + ser_no2)
            time.sleep(1)
            i = i + 1
            if i >= 5:
        ...
        logger.debug("type([0]seat_no): " + type(response.json()["content"][0]["seatno"]))
        ...
        for g in goodSeatNo:
            if (g in remain_seats) or (int(g) in remain_seats):
                seat_no = g
                logger.debug("str(type(g)): " + type(g))
                break
        # 没有选上中意的位置时，取剩余的中间位置
        if '0' == seat_no:
        ...
        i = 0
        while "排队中" in response1.json()["content"]:
            response1 = requests.post(url + '/seat/orderMySeat?mode=local', my_data)
            time.sleep(1)
            i = i + 1
            if i >= 5:
        ...
        if "排队成功！" in response1.json()["content"]:
            ...
            response2 = requests.post(
                ...
                headers=headers)
            i = 0
            while "排队中" in response2.json()["content"]:
                response2 = requests.post(
                    url + '/seat/get_task_status?serialno=' + str(check_serialno),
                    headers=headers)
                time.sleep(1)
